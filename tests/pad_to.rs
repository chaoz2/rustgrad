...
fn pad_to_right_pads_to_tinygrad_target_shape_and_keeps_trace_transparent() {
    ...
    let output = graph
        .pad_to_with_value(input, [Some(3), Some(4)], Scalar::F(-1.0))
        .unwrap();
...
fn pad_to_preserves_dtype_zero_domains_and_matching_shape_identity() {
    ...
    let output = bool_graph
        .pad_to_with_value(input, [Some(0), Some(3)], Scalar::Bool(true))
        .unwrap();
    assert_eq!(
        execute(
            &bool_graph,
    ...
    assert_eq!(
        identity_graph.pad_to(input, [Some(2), Some(3)]).unwrap(),
        input
    );
...
#[test]
fn pad_to_rejects_rank_mismatch_atomically_and_crops_smaller_targets() {
    let mut graph = Graph::new();
    let input = graph.input("input", [2, 3]);
    let before = graph.node_count();
    assert!(matches!(
        graph.pad_to(input, [Some(2)]),
        Err(Error::InvalidMovementRank { op: "pad_to", .. })
    ));
    assert_eq!(graph.node_count(), before);

    let cropped = graph.pad_to(input, [Some(1), Some(3)]).unwrap();
    assert_eq!(graph.shape(cropped).unwrap(), &Shape::from([1, 3]));
    assert_eq!(
        execute(&graph, cropped, data([2, 3], [1., 2., 3., 4., 5., 6.])),
        data([1, 3], [1., 2., 3.])
    );
}
...
fn pad_to_inherits_pad_reverse_mode_for_the_original_extent() {
    ...
    let padded = graph.pad_to(input, [Some(4)]).unwrap();
