...
impl Graph {
    ...
    /// tinygrad-compatible dot product.
    ///
    /// Unlike [`Self::matmul`], leading dimensions are not broadcast: they
    /// form an outer product. For example `[A, K].dot([B, K, N])` has shape
    /// `[A, B, N]`. This is represented through the existing static einsum
    /// engine, retaining its dtype and reverse-mode semantics.
