...
fn explicit_stream() -> RandomStream {
    RandomStream { device: 0, key: [0x14B8_1119, 0], counter: [0, 0] }
}
...
fn multinomial_replacement_is_explicit_stream_replayable_and_typed() {
    ...
    let sampled = graph.multinomial(weights, 4, -1, true, explicit_stream()).unwrap();
    let values = TensorData::from_scalars(
        [2, 3], DType::F32,
        [Scalar::F(1.0), Scalar::F(0.0), Scalar::F(2.0), Scalar::F(0.0), Scalar::F(3.0), Scalar::F(1.0)],
    ).unwrap();
    ...
    assert!(first.to_vec_f64().iter().all(|index| (0.0..3.0).contains(index)));
    assert!(graph.trace(sampled).unwrap().to_string().contains("tensor_guard"));
}
...
fn multinomial_without_replacement_uses_checked_stable_topk_indices() {
    ...
    let sampled = graph.multinomial(weights, 2, 0, false, explicit_stream()).unwrap();
    let values = TensorData::from_scalars([3], DType::F32, [Scalar::F(1.0), Scalar::F(2.0), Scalar::F(3.0)]).unwrap();
    let output = CpuBackend.execute(&graph, sampled, &HashMap::from([("weights".into(), values)])).unwrap();
...
fn multinomial_preflights_shape_count_and_dtype_without_graph_mutation() {
    ...
    assert!(graph.multinomial(integer, 1, 0, true, explicit_stream()).is_err());
    assert_eq!(graph.node_count(), before);
    let weights = graph.input_dtype("weights", [2, 3], DType::F32);
    let before = graph.node_count();
    assert!(matches!(graph.multinomial(weights, 4, 1, false, explicit_stream()), Err(Error::InvalidBounds { .. })));
    assert_eq!(graph.node_count(), before);
