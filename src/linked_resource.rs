...
use crate::{
    ...
    ptx::{LinkedF32ExpRequest, LINKED_F32_EXP_RENDERER_CONTRACT_VERSION},
...
impl LinkedF32ExpResourceDescriptor {
    pub fn from_request(
        ...
        let mut descriptor = Self {
            ...
            device: device.0,
            sm,
            inputs: request.link_inputs().iter().map(LinkInput::resource_descriptor).collect(),
        };
        descriptor.validate()?;
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, crate::ptx::PtxError> {
        let decoded = serde_json::from_slice::<Self>(bytes).map_err(|_| invalid("linked resource encoding"))?;
    ...
    pub fn rebind(
        ...
        let supplied = inputs.iter().map(LinkInput::resource_descriptor).collect::<Vec<_>>();
        if supplied != self.inputs {
            return Err(invalid("linked resource payload binding"));
        }
        Ok(LinkedF32ExpResourceBinding { descriptor: self.clone(), owner_identity: primary.identity() })
    }
    ...
    fn validate(&self) -> Result<(), crate::ptx::PtxError> {
        if self.schema_version != LINKED_F32_EXP_RESOURCE_SCHEMA_VERSION
            ...
            || self.inputs.iter().enumerate().any(|(index, input)| self.inputs[..index].iter().any(|prior| prior.name == input.name))
        {
    ...
    fn canonical_identity(&self) -> Result<String, crate::ptx::PtxError> {
        ...
        let bytes = serde_json::to_vec(&canonical).map_err(|_| invalid("linked resource encoding"))?;
        let hash = bytes.iter().fold(0xcbf29ce484222325_u64, |value, byte| {
            (value ^ u64::from(*byte)).wrapping_mul(0x100000001b3)
        });
        Ok(format!("linked-f32-exp-resource-v{}:{hash:016x}", self.schema_version))
    }
}

impl LinkedF32ExpResourceBinding {
    pub fn descriptor(&self) -> &LinkedF32ExpResourceDescriptor { &self.descriptor }
    pub fn validate_owner(&self, primary: &PrimaryContext) -> Result<(), crate::ptx::PtxError> {
        if self.owner_identity != primary.identity() { return Err(invalid("linked resource owner")); }
        Ok(())
    }
}
