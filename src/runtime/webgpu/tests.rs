...
fn portable_bitcast_wgsl_preserves_packed_raw_payload_and_bool_bytes() {
    ...
    assert!(
        rendered
            .source
            .contains(WGSL_PORTABLE_BITCAST_RENDERER_VERSION)
    );
    assert!(rendered.source.contains("atomicAnd(&b1[rg_word]"));
    ...
    assert!(
        renderer
            ...
            .unwrap()
            .source
            .contains("select(0u, 1u, rg_bits != 0u)")
    );
}
...
fn portable_sort_webgpu_executes_pairs_and_uses_atomic_bool_storage() {
    ...
    for dtype in [DType::Bool, DType::I32, DType::U32, DType::F32] {
        ...
        let items = crate::schedule_many(&graph, &[values, indices])
            .unwrap()
            .items;
        let rendered = renderer.render(&items[0].kernel).unwrap();
        rendered
        ...
        assert!(
            rendered
                .source
                .contains(WGSL_PORTABLE_SORT_RENDERER_VERSION)
        );
        if dtype == DType::Bool {
            assert!(rendered.source.contains("atomicAnd(&b1"));
            assert!(rendered.source.contains("atomicOr(&b1"));
    ...
    let mut graph = Graph::new();
    let input = graph.input_dtype("x", [3], DType::F32);
    let (values_id, indices_id) = graph.sort(input, 0, false).unwrap();
    let items = crate::schedule_many(&graph, &[values_id, indices_id])
        .unwrap()
        .items;
    let input_value =
        TensorData::from_storage([3], Storage::F32(vec![-0.0, 0.0, f32::NAN])).unwrap();
    let expected = crate::backend::stable_sort_pair(&input_value, 0, false).unwrap();
    let mock = Arc::new(MockDispatch::default());
    let (device, _) = setup(mock);
    let prefix = PreparedWebGpuPrefix::prepare(
        device,
        &items,
...
#[test]
fn prepared_webgpu_prefix_reuses_disjoint_exact_temporary_slots() {
    let renderer = WgslRenderer::new(8, capabilities()).unwrap();
    let mut graph = Graph::new();
    ...
    .unwrap();
    let prefix = PreparedWebGpuPrefix::from_plan(device, plan).unwrap();
    assert_eq!(
        mock.calls()
            .iter()
...
impl Dispatch for MockDispatch {
    ...
    fn launch(
            ...
            dispatch::KernelSemanticProgram::UOp(program) => match program.operation() {
                crate::Operation::Sort(value) => {
                    let input = bindings.get(value.input.index() as u64).ok_or_else(|| {
                        WebGpuError::InvalidBinding("sort semantic input absent".into())
                    })?;
        ...
        for (result, (output, logical)) in results.into_iter().zip(outputs) {
            let result = result
                .to_le_bytes()
                .map_err(|error| WebGpuError::InvalidBinding(error.to_string()))?;
...
fn renderer_identity_and_unsupported_work_are_pre_submission() {
    ...
    let reduced = graph
        ...
        .unwrap();
    let reduction = WgslRenderer::new(4, capabilities())
        .unwrap()
        .render(&schedule(&graph, reduced).unwrap().items[0].kernel)
        .unwrap();
    assert!(reduction.source.contains("for (var rg_r"));
    let mut unsupported = Graph::new();
    let f64_input = unsupported.input_dtype("x", [4], DType::F64);
    let f64_reduced = unsupported
        .reduce(f64_input, ReduceKind::Sum, Some(vec![0]), false)
        .unwrap();
    assert!(matches!(
        WgslRenderer::new(4, capabilities()).unwrap().render(&schedule(&unsupported, f64_reduced).unwrap().items[0].kernel),
        Err(WebGpuError::Unsupported(reason)) if reason.contains("F64")
    ));
...
#[test]
fn typed_reduction_recurrence_executes_through_webgpu_mock() {
    let cases = [
        (
            DType::F32,
            ReduceKind::Mean,
