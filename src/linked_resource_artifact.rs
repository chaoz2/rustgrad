...
impl LinkedF32ExpResourceArtifact {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, crate::ptx::PtxError> {
        ...
        if serde_json::to_vec(&artifact)
            ...
            != bytes
        {
            return Err(invalid("linked resource artifact unknown or noncanonical fields"));
        }
        artifact.validate()?;
        Ok(artifact)
    }
    ...
    pub fn rebind(
        ...
        let descriptor = descriptors.get(expected).ok_or_else(|| invalid("linked resource slot"))?;
        ...
        let payload = payloads.get(expected).ok_or_else(|| invalid("linked resource payload"))?;
        let binding = descriptor.rebind(primary, sm, payload)?;
        let bindings = BTreeMap::from([(slot.key.clone(), binding)]);
        Ok(BoundLinkedF32ExpResources { artifact: self.clone(), bindings })
    }
...
mod tests {
    ...
    fn linked_resource_sidecar_is_canonical_payload_free_and_tamper_checked() {
        ...
        assert_eq!(LinkedF32ExpResourceArtifact::decode(&bytes).unwrap(), artifact);
        assert!(!String::from_utf8(bytes.clone()).unwrap().contains("nvvm-payload"));
