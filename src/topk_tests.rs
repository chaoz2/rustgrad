...
fn topk_is_a_stable_sort_pair_followed_by_checked_slices() {
    ...
    let input = TensorData::from_scalars(
        [2, 4],
        DType::I32,
        [1, 1, 0, 1, 1, 3, 3, 2].map(Scalar::I),
    )
    .unwrap();
    assert_eq!(cpu(&graph, values, input.clone()).to_vec_f64(), vec![1., 1., 1., 3., 3., 2.]);
    assert_eq!(cpu(&graph, indices, input).to_vec_f64(), vec![0., 1., 3., 1., 2., 3.]);
    ...
    assert_eq!(schedule.items.iter().filter(|item| matches!(item.kernel.kind(), crate::UOpKind::Sort)).count(), 1);
    assert_eq!(schedule.items.iter().find(|item| matches!(item.kernel.kind(), crate::UOpKind::Sort)).unwrap().outputs.len(), 2);
}
...
fn topk_smallest_signed_axis_zero_and_zero_extent_preserve_static_contracts() {
    ...
    let input = TensorData::from_scalars([2, 3], DType::I16, [3, 1, 2, 0, 4, -1].map(Scalar::I)).unwrap();
    assert_eq!(cpu(&graph, values, input.clone()).to_vec_f64(), vec![0., 1., -1., 3., 4., 2.]);
    assert_eq!(cpu(&graph, indices, input).to_vec_f64(), vec![1., 0., 1., 0., 1., 0.]);

    let mut zero = Graph::new();
    let x = zero.input_dtype("x", [2, 3], DType::Bool);
    let (values, indices) = zero.topk(x, 0, 1, true).unwrap();
    let input = TensorData::from_scalars([2, 3], DType::Bool, [true, false, true, false, true, false].map(Scalar::Bool)).unwrap();
    assert_eq!(cpu(&zero, values, input.clone()).shape(), &Shape::new([2, 0]));
...
fn topk_preflights_axis_rank_and_k_without_graph_mutation() {
    ...
    assert!(matches!(
        ...
        Err(Error::InvalidBounds { axis: 1, end: 4, dim: 3, .. })
    ));
