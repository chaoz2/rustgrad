...
pub(crate) mod tests {
    ...
    fn linked_nvvm_inputs_are_ordered_and_distinct_from_library_bytes() {
        let mock = Arc::new(Mock::default());
        let context = context(&mock);
        let contract = NvvmProducerContract::new(
            11,
            8,
            1,
            20,
            90,
            vec![NvvmExportContract::new("__nv_expf".into(), NvvmPrototype::F32ToF32).unwrap()],
            b"bitcode",
        )
        .unwrap();
        let nvvm = LinkInput::nvvm("math.bc", b"bitcode".to_vec(), contract).unwrap();
