use super::{
    Conv1d, Conv1dOptions, ConvTranspose1d, ConvTranspose2d, Flatten, Linear, Module, Sequential,
};
use crate::{
    ConvTranspose1dOptions, ConvTranspose2dOptions, DType, Error, Graph, Result, TensorData,
    infer_module_cpu,
};
...
fn transpose1d_classifier(seed: u64, fixed: bool) -> Result<Sequential> {
    ...
    if fixed {
        ...
        linear
            .weight
            .replace(TensorData::new([1, 3], vec![1., 2., 3.])?)?;
        linear
            .bias
            .as_ref()
            .expect("configured bias")
...
#[test]
fn conv_transpose1d_static_constructor_and_module_forward_compose_deterministically() -> Result<()>
{
    let mut legacy_graph = Graph::new();
    let legacy = ConvTranspose1d::new(
        &mut legacy_graph,
        1,
        1,
        2,
    ...
    let target = transpose1d_classifier(83, false)?;
    let source_state = source.state_dict()?;
    let source_parameters = source
        .trainable_parameters()?
        .into_iter()
    ...
    let input = TensorData::new([2, 1, 2], vec![1., 2., 2., 1.])?;
    let first = infer_module_cpu(&target, input.clone())?;
    let second = infer_module_cpu(&target, input)?;
    assert_eq!(first.output().to_vec_f64(), [19., 16.]);
    assert_eq!(first.output(), second.output());
    assert_eq!(first.trace(), second.trace());
    assert_eq!(
        first.parameter_versions(),
...
fn conv_transpose1d_module_forward_preserves_empty_and_preflight_failure_contracts() -> Result<()> {
    ...
    assert!(matches!(
        infer_module_cpu(
            &model,
            TensorData::new([1, 1, 2], vec![1.; 2])?.cast(DType::F64)
        ),
        Err(Error::SessionTraining { .. })
    ));
    assert!(infer_module_cpu(&model, TensorData::new([1, 1], vec![1.])?).is_err());
    assert!(infer_module_cpu(&model, TensorData::new([1, 2, 2], vec![1.; 4])?).is_err());
    let mut unexpected = before.clone().into_tensors();
    unexpected.insert("1.weight".into(), TensorData::new([1], vec![1.])?);
    assert!(
        model
