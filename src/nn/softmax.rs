//! Stateless softmax module adapter.

use super::{Module, ModuleForward, Parameter, StateKind};
use crate::{DType, Graph, NodeId, Result};

/// A stateless stable softmax over one checked signed axis.
///
...
impl Softmax {
    /// Creates softmax using the input dtype for its graph calculation.
    pub const fn new(axis: isize) -> Self {
        Self { axis, dtype: None }
    }

    /// Creates softmax using the existing graph calculation/output dtype.
    ...
    pub fn forward(&self, graph: &mut Graph, input: NodeId) -> Result<NodeId> {
        graph.softmax(input, self.axis, self.dtype)
    }
}
...
impl ModuleForward for Softmax {
...
mod tests {
    ...
    #[test]
    fn softmax_is_stateless_and_matches_the_direct_graph_path() {
        let module = Softmax::new(-1);
        ...
        let direct_output = direct_graph.softmax(direct_input, -1, None).unwrap();
        ...
        assert_eq!(Softmax::with_dtype(1, DType::F32).dtype(), Some(DType::F32));
    }

    #[test]
    fn softmax_composes_in_static_cpu_inference_without_state_mutation() {
        let source_linear = Linear::new_static(2, 2, true, 101).unwrap();
        let mut source = Sequential::default();
        source.push(source_linear);
        source.push(Softmax::new(-1));
        let target_linear = Linear::new_static(2, 2, true, 107).unwrap();
        let mut target = Sequential::default();
        target.push(target_linear);
        target.push(Softmax::new(-1));
