...
| Convolution, pooling and Winograd | 🚧 | CPU-oracle NCHW/OIHW Conv2d supports optional channel bias, positive stride/dilation, asymmetric padding and groups/depthwise geometry. First-order float reverse mode is CPU-verified for input, weights and bias; pooling/Winograd, higher-order gradients, fuller dtype differential coverage and model parity remain. |
...
| Matmul/conv/attention gradients | 🚧 | First-order generalized matmul and Conv2d gradients are CPU-verified; attention gradients remain. |
