...
| Concrete redistribution routes | 🚧 | Phase 3A.2 emits deterministic layout-derived source/destination graph-buffer routes with ranks, exact element/byte ranges and dtype. The serializable CUDA plan consumes the route record directly; Driver submission remains pending. |
