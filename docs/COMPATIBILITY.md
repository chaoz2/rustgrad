...
| Executable CUDA plan companion | 🚧 | Two-owner shrink→binary composition retains PTX/plans with checked primary bindings. Executor byte-oracle comparison remains pending the mock `ViewBufferIndex` semantic binding; broader multi-owner acceptance remains pending. |
