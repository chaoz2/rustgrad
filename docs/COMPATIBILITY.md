...
| Indexing, slicing, gather/scatter and setitem | 🚧 | Checked shrink/signed slicing, integer gather, deterministic replacement scatter, and scatter-add exist. `ir::indexing` now provides a pure typed static mixed-index normalization boundary; Graph lowering/execution for that planner, general/fancy indexing and assignment remain. Dynamic boolean/nonzero cardinality and mutable alias semantics are separate boundaries. |
...
| Datasets and preprocessing | 🚧 | `datasets.rs` validates local uncompressed MNIST IDX image/label bytes and produces deterministic seeded batches. Downloading, caching, transforms, and other datasets remain. |
| MNIST and end-to-end training | 🚧 | A synthetic two-layer MLP workload proves deterministic loss decrease and uninterrupted/in-process-resumed agreement. It is not downloaded-MNIST accuracy or loss parity; real corpus training and evaluation remain. |
