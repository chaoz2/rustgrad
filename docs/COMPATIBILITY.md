...
| Matmul, batched matmul and linalg | 🚧 | CPU-oracle generalized matmul supports rank-1 dot, matrix/vector forms, and broadcasted batch dimensions with promoted dense dtypes. One validated `MatmulKernelPlan` owns its scheduled Matmul UOp payload, artifact identity, interpreter semantics, homogeneous F32/F64 CPU-JIT serial-K emission, and correctness-first PTX/mock semantic path. Dot/matvec/vecmat/broadcast batch/K=0 artifact replay and native cache reuse are differential-tested. Narrow native floats, bool/exact integers, tiling/shared memory/tensor cores, and live-CUDA validation remain explicit gaps. `MatmulGradVjp` retains second-order coverage; linalg beyond matmul and exhaustive accumulation-dtype parity remain. |
...
| Universal IR for ALU, memory, ranges and control | 🚧 | The immutable typed UOp DAG covers constants/vconsts, definitions, special/range/if delimiters, selected ALU/cast/vector/index/load/store/barrier/sink families, plus a validated static Matmul payload reusing normalized `MatmulKernelPlan`. Node-table artifacts preserve and validate the complete payload. Full tinygrad op vocabulary and broader kernel lowering remain. |
...
| Lazy realization and scheduling | 🚧 | Producer-aware schedule DAG/lazy realization records interpreter/native-JIT/fallback traces. Static matmul is an executable materialization root with ordered bindings; computed operands, dependencies, consumers, and temporary lifetimes remain explicit. Generation-checked HostSlotPool leases/views/detached outputs and exact-compatible MemoryPlan reuse are alias-safe. Late `LinearKernel`, `MemorySpacePlan`, and `VectorProgram` own portable elementwise lane/register legality. Target ISA SIMD lowering, workgroup/tiled matmul or reductions, and tensor cores remain boundaries. |
...
| Inspectable, serialized process replay | 🚧 | `CapturedSchedule` has deterministic bounded checksummed bytes for concrete and guarded symbolic-family schedule DAGs, ordered bindings, exact constants, shared typed UOp node tables, affine symbolic views, and exact-splat constant policies. Deserialized artifacts execute through explicit interpreter, strict native CPU-JIT, or caller-selected fallback without Graph reconstruction. `CapturedBatch` preflights and precompiles every same-family invocation before execution. Native elementwise/materialized-dependency, reduction, homogeneous F32/F64 generalized matmul, aligned contiguous views, legal strided scalar views, and vector scalar splats are CPU-oracle tested across multiple and zero-valued symbolic extents. Non-affine/negative-stride views, dynamic cardinality, native integer/narrow matmul, device resources, and CUDA graph integration remain. |
...
| JIT capture, specialization, cache and symbolic replay | 🚧 | `CapturedSchedule` serializes concrete schedules or guarded static-rank symbolic families, then replays through explicit interpreter/native/fallback policy without Graph traversal. Complete named bindings specialize checked UOp/schedule descriptors; canonical values key concrete specialization and process-local native compilation caches. Symbolic batch preflight validates and compiles every invocation before execution. Elementwise broadcast, static-axis reduction, generalized F32/F64 matmul, source-backed affine movement chains, and exact-splat constant resizing are differential-tested for multiple legal bindings and zero extents. External computed materializations remain explicit replay inputs. Dynamic rank/cardinality, arbitrary storage resizing, mutation aliases, serialized compiled code, tiled/device matmul, device resources, and CUDA graph integration remain absent. |
...
| Transformer/LLM, tokenizer, MLA and MoE | 🚧 | The checked-in GGUF `SimpleTokenizer` path has typed fail-closed metadata binding, the source pre-tokenizer preset allowlist/aliases, GPT-2 byte decoding, exact early-token and rank-ordered merge behavior, ordered specials, BOS/EOS/EOT behavior, Unicode/byte round trips, replacement decoding, and incremental UTF-8 decoding. Supported `general.architecture=llama` GGUF metadata constructs an explicit N-layer bias-free dense model, atomically materializes F32 state, and validates every fixed `blk.N` name/shape/dtype plus tied/explicit output. Its inspectable Graph/CPU path loops token embedding, RMSNorm, source q/k row permutation, positioned RoPE, causal GQA attention, residual gated FFN, final norm, and logits. Transactional per-layer caches have two-layer full/token/chunk parity at nonzero positions and match an independent dense oracle. The same graph path supports right-padded batches with independent positions/lengths, atomic all-row/all-layer cache commit, full/single parity, and uneven token/chunk parity. A staged CPU JIT path captures, serializes, decodes, and strictly native-replays every arithmetic/select/reduction/static-shrink/matmul node plus typed materializing concat, checked integer gather, replacement scatter, and homogeneous F32/F64 scatter-add. Reshape/permute/expand remain isolated movement-only oracle mini-graphs and visibly traced, never silent JIT fallback. Native two-layer logits match Graph and the independent oracle, native caches have full/token/chunk parity and compile-cache reuse, and a separately captured fixed batch preserves row isolation. Concrete sequence/batch shapes compile separate artifacts; symbolic/dynamic batching is not claimed. Native single/fixed-batch generation composes tokenizer text and checked Llama chat prompts through those caches, matches direct greedy and explicit-tape Gumbel token/text results, records stage/cache traces, handles independent EOS/EOT state, and rolls back cache/tokens on injected staged failure. Batch tapes remain explicit `[step,batch,vocabulary]` row-major values and deliberately make no tinygrad implicit-RNG parity claim. Absent chat metadata uses the checked-in Llama fallback formatter; one exact equivalent GGUF template string is accepted, and other Jinja/control templates are rejected. A serialized tiny GGUF fixture covers reader/materialization/binding/tokenizer/chat/model/batch-generation end to end. Generic SentencePiece/tokenizer.json models, empty special pieces, general Jinja/tools/multimodal chat, continuous/symbolic batching, other preset templates, bias/partial-RoPE/QK-norm/MLA/MoE/SSM and non-Llama variants, family-specific state rewrites, implicit Threefry replay, accelerated/native-quantized decoder execution, and representative external-model differential tests remain. |
