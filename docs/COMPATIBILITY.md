...
| Executable CUDA plan companion | 🚧 | Two-owner graph-composed axis-shard shrink→binary execution is mock-validated: original global leases bind via retained static view ABI, local outputs gather byte-for-byte to the CPU `ShardedGraphTensor` oracle, traces are deterministic, and repeated execution reuses owner-scoped semantic registrations. Redistribution failures, 1/4-owner matrices, further elementwise ops, collectives/reductions/matmul/movement diagnostics, and live CUDA remain pending. |
