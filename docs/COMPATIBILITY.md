...
| Transformer/LLM, tokenizer, MLA and MoE | 🚧 | The checked-in GGUF `SimpleTokenizer` path has typed fail-closed metadata binding, the source pre-tokenizer preset allowlist/aliases, GPT-2 byte decoding, exact early-token and rank-ordered merge behavior, ordered specials, BOS/EOS/EOT behavior, Unicode/byte round trips, replacement decoding, and incremental UTF-8 decoding. An explicit one-layer bias-free dense Llama binder atomically materializes F32 GGUF state and validates fixed names/shapes/dtypes/tied output. Its inspectable Graph/CPU path executes token embedding, RMSNorm, source q/k row permutation, positioned RoPE, causal GQA attention, residual gated FFN, final norm, and tied/explicit logits. A transactional KV cache has full-sequence/token-by-token parity at nonzero positions; a separate dense oracle and tokenizer prompt fixture agree. Generic SentencePiece/tokenizer.json models, empty special pieces, chat templates, multi-layer/config-metadata construction, batched inference, bias/QK-norm/MLA/MoE/SSM variants, generation/sampling, JIT and accelerated decoder execution, and representative external-model differential tests remain. |
