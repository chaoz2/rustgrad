...
| Transformer/LLM, tokenizer, MLA and MoE | 🚧 | The checked-in GGUF `SimpleTokenizer` path has typed fail-closed metadata binding, the source pre-tokenizer preset allowlist/aliases, GPT-2 byte decoding, exact early-token and rank-ordered merge behavior, ordered specials, BOS/EOS/EOT behavior, Unicode/byte round trips, replacement decoding, and incremental UTF-8 decoding. Supported `general.architecture=llama` GGUF metadata now constructs an explicit N-layer bias-free dense model, atomically materializes F32 state, and validates every fixed `blk.N` name/shape/dtype plus tied/explicit output. Its inspectable Graph/CPU path loops token embedding, RMSNorm, source q/k row permutation, positioned RoPE, causal GQA attention, residual gated FFN, final norm, and logits. Transactional per-layer caches have two-layer full/token/chunk parity at nonzero positions and match an independent dense oracle. Deterministic greedy generation and the checked-in Gumbel-max transform with an explicit uniform tape support EOS/EOT stopping, context rollback, and tokenizer prompt-to-decode fixtures. The tape deliberately avoids a tinygrad implicit-RNG parity claim. Generic SentencePiece/tokenizer.json models, empty special pieces, chat templates, batching, bias/partial-RoPE/QK-norm/MLA/MoE/SSM and non-Llama variants, family-specific state rewrites, implicit Threefry replay, JIT/accelerated/native-quantized decoder execution, and representative external-model differential tests remain. |
