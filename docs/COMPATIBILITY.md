...
| Executable CUDA plan companion | 🚧 | Graph-derived local Add has cache/oracle evidence across 1/2/4 owners for F32/I32/U64, including canonical zero routes. Static-view cast and broadcasted boolean select also execute through the generic semantic Mock across 1/2/4 owners, while Graph/CPU/autograd supports local Neg and cast. Logical zero bindings require no device pointer. Typed transfer→local composition substitutes exact descriptor-matched outputs into local inputs and preserves a validated dependency DAG; duplicate, descriptor, producer, and cycle errors are structured. PTX `GraphUnary`, computed-shrink broadcast, direct planner fusion, collectives, and live CUDA remain pending. |
...
| Tensor I/O, NumPy, zero-copy, disk/shm/tinyfs | 🚧 | Portable checked little-endian dense bytes and safetensors state dictionaries (in-memory and atomic filesystem APIs) are implemented. A fail-closed Torch ZIP state-dict importer accepts protocol-2 stored/raw-deflate CPU dense tensors through a no-code-execution pickle whitelist, with checked CRC, declared output/ratio bounds, and validated non-overlapping stride reconstruction into exact `TensorData` bits; its deterministic result converts to strict/non-strict module state loading. Single-disk ZIP64 central-directory and per-entry size/offset metadata is bounded and exact-extra-field validated. A separate in-memory ustar regular-file extractor rejects links/devices/path traversal and validates checksums. Legacy Torch TAR/pickle storage streams remain deliberately unsupported pending a record-oriented whitelist; CUDA/sparse/quantized/custom objects, ONNX, NumPy interop, and device-backed/file-backed lazy tensors remain separate milestones. |
...
| Optimized CPU renderer/compiler/JIT | 🚧 | `CpuJitBackend` provides an internal cached native execution boundary for eligible static kernels; schedule-DAG integration and broader differential coverage remain. |
...
| Optimizers and learning-rate schedules | 🚧 | Static host `Muon` joins SGD/Adam/AdamW/LARS/LAMB with checked rank/float/config validation, momentum/Nesterov, and tinygrad-style Newton--Schulz matrix orthogonalization; independent square/tall/wide, resume, re-quantization, and invalid-input evidence is checked in. Existing optimizer checkpoints retain config fingerprints, legacy rejection, and strict atomic expected-key loads; LAMB uses corrected bias correction. OptimizerGroup, schedulers, fused/device paths, distributed state, and loss/gradient scaling remain. |
| State traversal, safetensors, GGUF and checkpoints | 🚧 | Explicit deterministic `Module` traversal covers nested `Sequential`, buffers, tied parameters, strict/non-strict reports, fixed shape/dtype replacement, and safetensors round trips. The bounded Torch CPU state-dict ZIP subset is imported into the same map without mutation; full Torch checkpoint compatibility, GGUF, and full ecosystem state traversal remain. |
