...
| Supported higher-order composition | 🚧 | Smooth unary/binary expressions, broadcasting, ordinary movement, sum/mean, select value branches, generalized matmul and normalized-plan einsum support graph-on-graph second derivatives. Third-order indexed contractions, `ReduceGrad` product/extrema, scatter-position/gather/scatter-add, masked selection, convolution, transpose convolution, and pooling-derived dedicated gradients remain explicit boundaries. |
