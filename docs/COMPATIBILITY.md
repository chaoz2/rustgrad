...
| Transformer/LLM, tokenizer, MLA and MoE | 🚧 | The checked-in GGUF `SimpleTokenizer` path has typed fail-closed metadata binding, the source pre-tokenizer preset allowlist/aliases, GPT-2 byte decoding, exact early-token and rank-ordered merge behavior, ordered specials, BOS/EOS/EOT behavior, Unicode/byte round trips, replacement decoding, and incremental UTF-8 decoding. An explicit one-layer dense Llama state binder atomically materializes F32 GGUF state, validates the fixed tensor inventory/shapes/dtypes and optional rope frequencies, and distinguishes explicit from tied output weights. Generic SentencePiece/tokenizer.json models, empty special pieces, chat templates, multi-layer/config-driven schemas, RoPE weight conversion, graph construction, decoder execution, generation, MLA, MoE, and representative external-model differential tests remain. |
