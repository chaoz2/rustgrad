...
| Supported higher-order composition | 🚧 | Smooth unary/binary expressions, broadcasting, ordinary movement, sum/mean/product/max/min, gather/scatter-add, select value branches, generalized matmul and normalized-plan einsum support graph-on-graph second derivatives. Third-order indexed contractions/reductions/movement, fixed-size masked selection, replacement scatter, convolution, transpose convolution, and pooling-derived dedicated gradients remain explicit boundaries. |
