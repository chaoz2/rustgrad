...
| Transformer/LLM, tokenizer, MLA and MoE | 🚧 | The checked-in GGUF `SimpleTokenizer` path has typed fail-closed metadata binding, the source pre-tokenizer preset allowlist/aliases, GPT-2 byte decoding, exact early-token and rank-ordered merge behavior, ordered specials, BOS/EOS/EOT behavior, Unicode/byte round trips, replacement decoding, and incremental UTF-8 decoding. Supported `general.architecture=llama` GGUF metadata constructs an explicit N-layer bias-free dense model, atomically materializes F32 state, and validates every fixed `blk.N` name/shape/dtype plus tied/explicit output. Its inspectable Graph/CPU path loops token embedding, RMSNorm, source q/k row permutation, positioned RoPE, causal GQA attention, residual gated FFN, final norm, and logits. Transactional per-layer caches have two-layer full/token/chunk parity at nonzero positions and match an independent dense oracle. The same graph path supports right-padded batches with independent positions/lengths, atomic all-row/all-layer cache commit, full/single parity, and uneven token/chunk parity. Deterministic greedy generation and the checked-in Gumbel-max transform support independent EOS/EOT state; batch tapes are explicit `[step,batch,vocabulary]` row-major values and deliberately make no tinygrad implicit-RNG parity claim. Absent chat metadata uses the checked-in Llama fallback formatter; one exact equivalent GGUF template string is accepted, and other Jinja/control templates are rejected. A serialized tiny GGUF fixture covers reader/materialization/binding/tokenizer/chat/model/batch-generation end to end. Generic SentencePiece/tokenizer.json models, empty special pieces, general Jinja/tools/multimodal chat, continuous batching, other preset templates, bias/partial-RoPE/QK-norm/MLA/MoE/SSM and non-Llama variants, family-specific state rewrites, implicit Threefry replay, JIT/accelerated/native-quantized decoder execution, and representative external-model differential tests remain. |
