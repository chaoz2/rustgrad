...
| Tensor I/O, NumPy, zero-copy, disk/shm/tinyfs | 🚧 | Portable checked little-endian dense bytes and safetensors state dictionaries (in-memory and atomic filesystem APIs) are implemented. A fail-closed Torch ZIP state-dict importer accepts only protocol-2, stored ZIP CPU dense tensors through a no-code-execution pickle whitelist, reconstructing validated non-overlapping strides into exact `TensorData` bits; its deterministic result converts to strict/non-strict module state loading. Compressed/ZIP64/TAR/legacy Torch archives, CUDA/sparse/quantized/custom objects, ONNX, NumPy interop, and device-backed/file-backed lazy tensors remain separate milestones. |
