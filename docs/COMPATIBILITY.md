...
| Broadcast/reduction/view gradients | 🚧 | Broadcast/reduction and finite-difference checks exist, including selection gradients through value branches plus product/extrema gradients across multi-axis and keepdim cases; predicates are intentionally nondifferentiable. Sum/mean, zero-aware product, max/min tie routing, reshape/permute/expand/shrink/pad/stride/concat and select value branches retain second-order graph edges. `ReduceGradVjp` preserves product zero-count branches and extrema upstream routing while treating comparison masks as input-nondifferentiable. Scatter-position, gather/scatter-add, and masked selection do not yet have a higher-order rule. |
| Matmul/conv/attention gradients | 🚧 | Generalized `MatmulGradVjp` and normalized-plan `EinsumGradVjp` provide second-order VJPs/HVPs for dot, vector/matrix, batched/broadcast matmul and einsum value contractions, including repeated labels. Convolution/transposed-convolution gradients and pooling-derived dedicated gradients remain first-order-only. Compositional attention paths backpropagate through query/key/value and are covered by finite fixtures. |
...
| Supported higher-order composition | 🚧 | Smooth unary/binary expressions, broadcasting, ordinary movement, sum/mean/product/max/min, select value branches, generalized matmul and normalized-plan einsum support graph-on-graph second derivatives. Third-order indexed contractions/reductions, scatter-position/gather/scatter-add, masked selection, convolution, transpose convolution, and pooling-derived dedicated gradients remain explicit boundaries. |
