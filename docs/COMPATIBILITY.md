...
| Convolution, pooling and Winograd | 🚧 | CPU-oracle NCHW/OIHW Conv2d is acceptance-tested. Static trailing-spatial max/average pool2d supports kernel/stride/dilation/padding/ceil mode and both average border-divisor policies through inspectable primitive window compositions, with max gradients inherited from the tested reduction transform. Checked-in tinygrad also exposes max `return_indices` (flattened original-spatial index, earliest-index tie convention); RustGrad has not yet added that separate index-output API. First-class Pool IR is deliberately unnecessary while transparent movement/reduction lowering preserves value/gradient behavior. Exhaustive pooling dtype/NaN parity and Winograd remain. |
