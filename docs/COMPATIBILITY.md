...
| Executable CUDA plan companion | 🚧 | Graph-derived local Add has cache/oracle evidence across 1/2/4 owners for F32/I32/U64, including canonical zero routes. Unary is diagnostic-only; select condition shape, computed-shrink broadcast, cast wrapping, graph-derived redistribution/failures, collectives, and live CUDA remain pending. |
| Concrete redistribution routes | 🚧 | Phase 3A.2 emits deterministic layout-derived source/destination graph-buffer routes with ranks, exact element/byte ranges and dtype. The executor has mock-validated same-owner DtoD and cross-owner peer submission from canonical routes, with exact copied bytes and deterministic ordering. Full layout-derived multi-owner acceptance remains pending. |
...
| Lazy realization and scheduling | 🚧 | Producer-aware schedule DAG/lazy realization records interpreter/native-JIT/fallback traces. Generation-checked HostSlotPool leases/views/detached outputs and exact-compatible MemoryPlan reuse are alias-safe; backend-owned slots and vector/lane byte windows remain boundaries. |
...
| Optimizers and learning-rate schedules | 🚧 | Static `LARS` and `LAMB` join SGD/Adam/AdamW with checked config fingerprints, legacy rejection, strict atomic expected-key checkpoint loads, and independent one/two-step/resume evidence; LAMB uses corrected bias correction. Muon/Newton-Schulz, OptimizerGroup, schedules, fused/device paths, gradient scaling, and distributed state remain. |
