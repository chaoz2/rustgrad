...
| Creation and seeded random generation | 🚧 | Static dense `full/zeros/ones/empty`, integer `arange`, `linspace`, and `eye`, all static `*_like` forms, `randperm`, and scaled/Glorot/Kaiming uniform/normal initializers are CPU-tested. Replayable graph random nodes use explicit per-node SplitMix64 seeds; atomic `manual_seed` plus `*_implicit` calls provides a safe serialized compatibility façade. This deliberately differs from tinygrad's stateful per-device Threefry bytes. `one_hot`/`meshgrid` require the remaining public rank-expansion conveniences, while random `*_like` implicit variants, Threefry bit compatibility, permutations beyond randperm, and distribution initializers outside this list remain. RustGrad `empty` deterministically zero-initializes instead of exposing allocator contents. |
