...
| Executable CUDA plan companion | 🚧 | Graph-derived local Add has cache/oracle evidence across 1/2/4 owners for F32/I32/U64, including canonical zero routes. Logical zero bindings require no device pointer. Typed transfer→local composition substitutes exact descriptor-matched outputs into local inputs and preserves a validated dependency DAG; duplicate, descriptor, producer, and cycle errors are structured. Unary is diagnostic-only; select condition shape, computed-shrink broadcast, cast wrapping, direct planner fusion, collectives, and live CUDA remain pending. |
...
| Lazy realization and scheduling | 🚧 | Producer-aware schedule DAG/lazy realization records interpreter/native-JIT/fallback traces. Generation-checked HostSlotPool leases/views/detached outputs and exact-compatible MemoryPlan reuse are alias-safe. Late `LinearKernel` builds a typed portable contiguous lane plan for JIT C main/tail rendering; target vector instructions/register allocation and backend-owned slots/vector byte windows remain boundaries. |
