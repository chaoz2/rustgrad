...
| OpenCL runtime and OpenCL C rendering | 🚧 | Runtime-loaded OpenCL 1.2 ICD with exact symbol errors; thread-confined RAII resources; checked copies/build logs/owner and capability-aware cache/launch preflight. Static OpenCL C and owner-scoped semantic mock cover contiguous/broadcast plus checked strided static shrink views; Bool/I32/U32/F32 and capability-gated I64/U64/F64; and fp64-gated raw F16/BF16 with software exact decode/ties-to-even encode rather than native-half dependence. Narrow literals, loads/stores, strided views, float-family casts, wrapping arithmetic/compare/select, signed zero/subnormal/Inf/NaN, and Sum/Mean/Product/Min/Max have raw-byte oracle coverage. Versioned staged execution covers one root guarded I32/U32/I64/U64 division/modulo/floor-division/shift: atomic earliest-index status, retained scratch/status/events, exact shift-detail reconstruction, and success-only commit after clean status. Compute/status/commit-submission failures preserve destination bytes; a native failure after commit-copy acceptance cannot promise rollback without indirect buffer identity. I64/U64 extrema additionally require fp64 to reproduce CPU f64-projection ordering. Multi-axis/keepdim/empty identities and ordered binding/view/reduction/capability/transaction metadata participate in ABI/source identity; native execution has no host fallback. Multiple/computed guarded expressions, broader unary ops, runtime-polymorphic views/shapes, cross-thread resources, and broad live-device validation remain. |
