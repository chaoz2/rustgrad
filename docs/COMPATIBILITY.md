...
| Inspectable, serialized process replay | 🚧 | `CapturedSchedule` has deterministic bounded checksummed bytes for concrete static schedule DAGs, ordered bindings, exact constants, and shared typed UOp node tables. Deserialized artifacts execute through explicit interpreter, strict native CPU-JIT, or caller-selected fallback without Graph reconstruction. `CapturedBatch` preflights every same-artifact invocation before execution. Scalar native elementwise/materialized-dependency, reduction, and homogeneous F32/F64 generalized-matmul paths are CPU-oracle tested; static views require explicit fallback. Symbolic specialization, native integer/narrow matmul, device resources, and CUDA graph integration remain. |
...
| JIT capture, specialization, cache and symbolic replay | 🚧 | `CapturedSchedule` serializes concrete static schedule UOps with ordered bindings and exact constants, then replays through explicit interpreter/native/fallback policy without Graph traversal. Native compilation consumes retained UOps/descriptors and stays process-local. Generalized F32/F64 Matmul payloads support deterministic single and batched invocation cache reuse. External computed materializations remain explicit replay inputs. Symbolic runtime specialization, serialized compiled code, tiled/device matmul, device resources, and CUDA graph integration remain absent. |
