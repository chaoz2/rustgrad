...
| Inspectable, serialized process replay | 🚧 | `CapturedSchedule` has deterministic bounded checksummed bytes for concrete and guarded symbolic-family schedule DAGs, ordered bindings, exact constants, and shared typed UOp node tables. Deserialized artifacts execute through explicit interpreter, strict native CPU-JIT, or caller-selected fallback without Graph reconstruction. `CapturedBatch` preflights and precompiles every same-family invocation before execution. Native elementwise/materialized-dependency, reduction, and homogeneous F32/F64 generalized-matmul paths are CPU-oracle tested across multiple and zero-valued symbolic extents; static and symbolic views require explicit fallback or capture-time rejection. Dynamic cardinality, native integer/narrow matmul, device resources, and CUDA graph integration remain. |
...
| Optimized CPU renderer/compiler/JIT | 🚧 | `CpuJitBackend` provides cached native execution for Graph outputs and a validated `ScheduleItem` seam used by serialized replay. Process-local compilation is keyed by deterministic rendered identity and repeated artifact replay reports cache hits. Scalar/contiguous-vector elementwise, reductions, and homogeneous F32/F64 generalized matmul cover zero domains, vector tails, broadcast batches, and multi-item dependencies. Static views, native narrow/integer matmul, tiling, and unproven vector scalar broadcasts remain explicit unsupported/fallback boundaries. |
