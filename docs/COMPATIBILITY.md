...
| Mask, pad, concat, stack and split | 🚧 | Constant typed padding, multi-input concat with promotion, and fixed-size `masked_select(size, fill)` exist. A typed dynamic-result foundation and CPU `nonzero` are available; unbounded boolean-mask selection remains the next dynamic consumer because its F32 reverse scatter needs a dynamic-upstream autograd boundary. Stack and split remain. |
...
| OpenCL runtime and OpenCL C rendering | 🚧 | Runtime-loaded OpenCL 1.2 ICD with exact symbol errors; thread-confined RAII platform/device/context/queue/buffer/program/kernel/event ownership; checked copies, bounded build logs, context/device/options cache identity, ordered pointer-plus-extent launch ABI, and deterministic owner-scoped semantic mock differential for static broadcast F32 elementwise/select and zero domains. An ignored Apple OpenCL smoke verifies live F32 Add build/transfer/launch bytes on the development host. Views, reductions, broader dtypes/ops, runtime-polymorphic shapes, and cross-platform live validation remain. |
