...
| Convolution, pooling and Winograd | 🚧 | CPU-oracle NCHW/OIHW Conv2d is acceptance-tested. General static trailing-spatial `max_pool`/`avg_pool` support arbitrary rank options, including checked-in 3D tuple usage; 2D compatibility wrappers retain their existing API. Generalized max indices return I32 flattened original-spatial indices with earliest-index ties, while differentiable max values preserve split-tie gradients. Pooling behavior is validated from checked-in source and RustGrad's CPU oracle; a live tinygrad runtime differential remains unavailable in this workspace. Winograd and broader convolution-family work remain. |
