...
| Tensor I/O, NumPy, zero-copy, disk/shm/tinyfs | 🚧 | Portable checked little-endian dense bytes and safetensors state dictionaries (in-memory and atomic filesystem APIs) are implemented. Full state traversal/loading, Torch pickle, ONNX, NumPy interop, and device-backed/file-backed lazy tensors remain separate milestones. |
