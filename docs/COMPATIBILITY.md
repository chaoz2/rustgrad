...
| Lazy realization and scheduling | 🚧 | Deterministic non-mutating schedule view has typed buffer descriptors, checked byte sizes, stable cache keys, and scalar/rank-N pure-elementwise UOp lowering. Unsupported graph operations remain explicit boundaries. Reductions, memory reuse and device realization remain. |
| Fusion, range lowering and indexing | 🚧 | Owned typed runtime bindings, checked element/byte addressing, row-major scalar/zero/rank-N domains, broadcast offset maps, symbolic shape specialization, and an independent range/load/store elementwise interpreter are tested against the CPU oracle. Reduction axes, allocation reuse and renderer linearization remain. |
