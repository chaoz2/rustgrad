...
| Inspectable, serialized process replay | 🚧 | `CapturedSchedule` has deterministic bounded checksummed bytes for concrete static schedule DAGs, ordered bindings, exact constants, and shared typed UOp node tables. Deserialized artifacts execute through explicit interpreter, strict native CPU-JIT, or JIT-with-interpreter-fallback policies without Graph reconstruction. `CapturedBatch` preflights every same-artifact invocation before execution and returns ordered traces with owned outputs. Scalar native elementwise/materialized-dependency and reduction paths are CPU-oracle tested; static views require explicit fallback. Symbolic specialization, matmul lowering, device resources, and CUDA graph integration remain. |
...
| Optimized CPU renderer/compiler/JIT | 🚧 | `CpuJitBackend` provides cached native execution for Graph outputs and a validated `ScheduleItem` seam used by serialized replay. Process-local compilation is keyed by deterministic rendered identity and repeated artifact replay reports cache hits. Scalar and contiguous-vector static elementwise plus reductions, including vector tails, zero domains, and multi-item dependencies, are covered. Pure `rangeify` static shrink planning owns nested storage-offset normalization, but native static-view artifact execution and unproven vector scalar broadcasts remain explicit unsupported/fallback boundaries. |
...
| JIT capture, specialization, cache and symbolic replay | 🚧 | `CapturedSchedule` serializes concrete static schedule UOps with ordered bindings and exact constants, then replays through explicit interpreter/native/fallback policy without Graph traversal. Native compilation consumes retained UOps and descriptors, translates schedule operand order to the renderer ABI, and stays process-local. External computed materializations are explicit replay inputs; immutable same-artifact batches preflight all invocations before ordered execution. Symbolic runtime specialization, serialized compiled code, matmul UOp lowering, device resources, and CUDA graph integration remain absent. |
