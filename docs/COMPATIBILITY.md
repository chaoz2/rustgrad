...
| Convolution, pooling and Winograd | 🚧 | CPU-oracle NCHW/OIHW Conv2d is acceptance-tested. Static trailing-spatial max/average pool2d supports kernel/stride/dilation/padding/ceil mode through inspectable primitive window compositions, with max gradients inherited from the tested reduction transform. `count_include_pad=false`, max `return_indices`, first-class Pool IR, and exhaustive pooling dtype/NaN parity remain. Winograd and higher-order gradients remain. |
