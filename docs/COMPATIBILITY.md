...
| State traversal, safetensors, GGUF and checkpoints | 🚧 | Explicit deterministic `Module` traversal covers nested `Sequential`, buffers, tied parameters, strict/non-strict reports, fixed shape/dtype replacement, and safetensors round trips. Graph-independent stable host `Parameter` identities bind versioned leaves per Graph. `TrainingCheckpoint` remains the strict same-identity in-process path. The separate bounded `PortableTrainingCheckpoint` restores module values/versions, tie topology, one `Optimizer`'s parameter-group ownership/state/LRs, and one scheduler into fresh process-local identities with staged validation and deterministic bytes. The bounded in-memory GGUF v2/v3 reader preserves typed metadata, validates dense and source-evidenced quantized tensor descriptors/ranges, and materializes exact dense F32/F16/I8/I16/I32/I64/F64/BF16 storage; quantized payloads remain opaque. Optimizer-group orchestration, Graph/device state, GGUF dequantization/model loading/split files/LLM execution, full Torch checkpoint compatibility, and full ecosystem state traversal remain. |
