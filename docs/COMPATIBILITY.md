...
| Creation and seeded random generation | 🚧 | Static dense `full/zeros/ones/empty`, integer `arange`, `linspace`, and `eye` are CPU-tested. Replayable graph `rand/uniform/randn/normal/randint` nodes use explicit per-node SplitMix64 seeds, preserving deterministic traces and scheduling independence. This deliberately differs from tinygrad's stateful per-device Threefry stream; `*_like`, global `manual_seed`, Threefry bit compatibility, Bernoulli, permutations, and distribution initializers remain. RustGrad `empty` deterministically zero-initializes instead of exposing allocator contents. |
