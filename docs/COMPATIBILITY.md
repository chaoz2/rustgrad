...
| Optimizers and learning-rate schedules | 🚧 | Static host `Muon` joins SGD/Adam/AdamW/LARS/LAMB with checked rank/float/config validation, momentum/Nesterov, and tinygrad-style Newton--Schulz matrix orthogonalization. `TrainingCheckpoint` provides identity/version-preserving in-process resume with safetensors module bytes plus optimizer/scheduler states; the synthetic IDX MLP matches uninterrupted parameters, slots, LR/epoch, loss, and inference bytes exactly, and cross-part failures are atomic. Existing optimizer checkpoints retain config fingerprints, legacy rejection, and strict expected-key loads; LAMB uses corrected bias correction. OptimizerGroup, fused/device paths, distributed state, and loss/gradient scaling remain. |
