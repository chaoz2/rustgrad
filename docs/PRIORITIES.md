...
| Manual scale probe | An exact-commit, manual-only workflow runs a larger bounded `[4,8]`, two-block strict-native CPU workload. It records the existing scoreboard plus loss, progress, pending-window checkpoint, and exact in-place resume evidence; timings remain observational and failures select future compiler work. |
