...
`Embedding`, `Dropout`, `LayerNorm`, `Conv1d`, `Conv2d`, `ConvTranspose1d`, `ConvTranspose2d`, `AvgPool2d`, `AdaptiveAvgPool2d`, `MaxPool2d`, or
`Flatten` entries,
...
`AvgPool2d`, `AdaptiveAvgPool2d`, `LayerNorm`, and `Flatten::new(start_dim)` also compose for the
verified static chains. BatchNorm lifecycle, `LayerNorm2d`, RMSNorm, other pooling, normalization, stateful, and
